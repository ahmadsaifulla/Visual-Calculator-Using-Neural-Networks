...
def build_model(config: ArchitectureConfig, num_classes: int) -> tf.keras.Model:
    """Build an ANN using only Flatten, Dense, and Dropout layers."""
...
def train_with_retries(x_train: np.ndarray, y_train: np.ndarray, x_test: np.ndarray, y_test: np.ndarray) -> tuple[tf.keras.Model, tf.keras.callbacks.History, float]:
    ...
    for index, config in enumerate(ARCHITECTURE_RETRIES, start=1):
        logger.info("Training digit ANN attempt %s with hidden=%s dropout=%.2f", index, config.hidden_units, config.dropout)
...
def main() -> None:
    ...
    save_confusion(y_true, y_pred, DIGIT_CLASSES, Path(__file__).resolve().parent / "digit_confusion_matrix.png")
